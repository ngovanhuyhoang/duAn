...
=======
        ...
        {
            if (cmbx_chonhd.SelectedItem == null || cmbx_chonhd.SelectedItem.ToString().Trim() == "")
            {
                MessageBox.Show("Vui lòng chọn một hóa đơn để thanh toán.");
                return;
            }

            string maHoaDon = cmbx_chonhd.SelectedItem.ToString();

            using (var countdownForm = new CountDown(maHoaDon))
            {
